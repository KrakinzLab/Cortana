...
function printLog(isCmd, sender, msg, body, groupName, isGc) {
	...
	if (isCmd && !isGc) {
		return console.log(color("[ CORTANA MD ]", "aqua"), color(sender.split("@")[0], "lime"), color(body, "aqua"));
	}
}
	...
	try {
		...
		conn.sendMessage = async (jid, content, options = { isTranslate: true }) => {
			...
			if (
				...
				fromContent.key.id = "CORTANA" + require("crypto").randomBytes(13).toString("hex").toUpperCase();
